package com.example.fitfinessee.JadwalWoPage.bottomSheetFragment;
...
public class CreateTaskBottomSheetFragment extends BottomSheetDialogFragment {
    ...
    public void setTaskId(int taskId, boolean isEdit, setRefreshListener setRefreshListener, MainActivityJadwal activity) {
        this.taskId = taskId;
        this.isEdit = isEdit;
    ...
    public boolean validateFields() {
        if(addTaskTitle.getText().toString().equalsIgnoreCase("")) {
            ...
            Toast.makeText(activity, "Masukkan deskripsi yang valid", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskDate.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity, "Masukkan tanggal yang valid", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskTime.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity, "Masukkan waktu yang valid", Toast.LENGTH_SHORT).show();
            return false;
        }
        else if(taskEvent.getText().toString().equalsIgnoreCase("")) {
            Toast.makeText(activity, "Masukkan tugas yang valid", Toast.LENGTH_SHORT).show();
            return false;
        }
        else {
            return true;
    ...
    private void createTask() {
        class saveTaskInBackend extends AsyncTask<Void, Void, Void> {
            ...
            protected void onPostExecute(Void aVoid) {
                ...
                Toast.makeText(getActivity(), "Tugas Anda telah ditambahkan", Toast.LENGTH_SHORT).show();
                dismiss();
    ...
    public void createAnAlarm() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                PendingIntent intent = PendingIntent.getBroadcast(activity, count, alarmIntent, PendingIntent.FLAG_UPDATE_CURRENT);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
